 ...
 *
 * Spawns `pi --mode rpc --no-session`, runs 2 iterations with
 * `new_session` reset between them, renders output using pi's
...
// ToolExecutionComponent needs a TUI instance for requestRender() on
// async image conversion — stub it since we only do static rendering.
...
function runDemo(
	...
	const eventsStream = createWriteStream(join(ralphDir, "events.jsonl"), { flags: "w" });
