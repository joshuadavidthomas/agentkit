...
Error strategy and design for Rust. Covers the library-vs-application decision,
structured error enums with thiserror, ergonomic propagation with anyhow, Result/Option
combinators, when to panic, and error boundary rules.

## Scope

Use this skill when designing error types, choosing between thiserror and anyhow,
writing error propagation code, or deciding how errors should cross abstraction
boundaries. The central axis is **library vs application** — different contexts
...
Standard library evidence examples referenced: `io::Error`, `io::ErrorKind`,
`num::ParseIntError`, `serde_json::Error`, `fmt::Result`, `io::Result`.
