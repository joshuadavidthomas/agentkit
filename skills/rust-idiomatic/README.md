...
The paradigm shift skill for Rust. Encodes the core "think in Rust" defaults that
AI agents consistently get wrong: newtypes for domain strings, enums over booleans,
exhaustive matching, structured errors, parse-don't-validate, enums as the primary
modeling tool, and borrow-by-default.

The `SKILL.md` provides 9 hard rules with code examples, ecosystem citations, and a
10-point review checklist. The `references/` directory contains deep-dive content on
specific patterns: newtypes, enums-as-modeling-tool, and boundary parsing.
