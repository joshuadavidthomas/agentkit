...
Fix: Trust readers to remember what they just read. Summarize only when the audience genuinely needs it (executive summaries, abstracts).
...
Fix: Use a metaphor once or twice, then let it go. If you need it again later, it should feel like a callback, not a crutch.
...
Fix: Pick one example and go deep. A single well-analyzed case is worth more than five name-drops.
...
- "Each section rephrases the thesis with a different metaphor but adds nothing
  new"

Fix: State the point. Support it. Move on. If a piece only has one idea, it should be short.
...
Fix: Re-read what you've written. Cut duplicates.
...
Fix: Just write your final thought. Drop the signpost.
...
Fix: If challenges matter, discuss them seriously. If they don't, skip them. Don't use them as a rhetorical speed bump on the way to optimism.
