...
def main():
    ...
    project_research = Path.cwd() / ".agents" / "research"
    global_research = Path.home() / ".agents" / "research"
    ...
    if not is_project and not is_global:
        print(
            "Error: Path must be in a research directory (.agents/research/ or ~/.agents/research/)",
            file=sys.stderr,
        )
        sys.exit(1)
