...
The `SKILL.md` is a prescriptive rulebook: when unsafe is justified, how to contain it behind safe APIs, mandatory `# Safety` and `// SAFETY:` documentation requirements, the UB categories you must prevent, and a review checklist. The `references/` directory contains deeper guidance on documenting unsafe contracts, UB/validity obligations, FFI boundary defaults, and validating unsafe code with Miri and complementary tooling.
...
- [cbindgen](https://github.com/mozilla/cbindgen) — Generating C headers from Rust `extern "C"` APIs. Licensed under [MPL-2.0](https://www.mozilla.org/en-US/MPL/2.0/).
- [rust-bindgen book](https://rust-lang.github.io/rust-bindgen/) — Generating Rust bindings from C headers (FFI boundary workflows). Licensed under [MIT](https://opensource.org/licenses/MIT) OR [Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0).
