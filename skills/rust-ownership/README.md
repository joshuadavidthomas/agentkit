...
Ownership, borrowing, and lifetimes skill for Rust. Encodes the design thinking behind the borrow checker — when to borrow, when to own, when to clone, and how to choose smart pointers. Activated by borrow checker errors (E0382, E0505, E0597, etc.) and function signature design questions.

The `SKILL.md` provides an error-to-design-question table, function signature rules, a smart pointer decision tree, lifetime essentials, guidance on when clone is fine, and a 10-point review checklist. The `references/` directory contains deep-dive content on smart pointers, lifetime patterns, and function signature design.
